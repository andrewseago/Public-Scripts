...
if [ "$installType" = "" ]; then
	if [ "$policyName" != "DMG" ] && [ "$policyName" != "fut" ] && [ "$policyName" != "feu" ] && [ "$policyName" != "feu,fut" ] && [ "$policyName" != "pkg" ] ; then
		declare -x installType="$6"
	else
		declare -x installType="$policyName"
	fi
fi
...
function cleanup () {
		...
		if [ "$postTrigger" != "" ] && [ "$installStatus" = "Complete" ]; then
			...
			$jamf_binary policy -trigger "$postTrigger" >> $logFile
			if [ $? == 0 ]; then
...
function installDMG () {
	if [ "$installType" == "DMG" ]; then
		...
		$jamf_binary install -package "$dmgName" -path "$pathToDMG"  -target / -progress -verbose >> $logFile
	fi

	####	fut (dmg with "Fill user templates") Installer
	if [ "$installType" == "fut" ]; then
		log "$installType Install"
		$jamf_binary install -package "$dmgName" -path "$pathToDMG"  -target / -progress -verbose -fut >> $logFile
	fi
	####	feu (dmg with "Fill existing users") installer
	if [ "$installType" == "feu" ]; then
		log "$installType Install"
		$jamf_binary install -package "$dmgName" -path "$pathToDMG"  -target / -progress -verbose -feu >> $logFile
	fi
	####	feu,fut (dmg with both "Fill existing users" and "Fill user templates") installer
	if [ "$installType" == "feu,fut" ]; then
		log "$installType Install"
		$jamf_binary install -package "$dmgName" -path "$pathToDMG"  -target / -progress -verbose -feu -fut >> $logFile
	fi
	if [ $? == 0 ]; then
